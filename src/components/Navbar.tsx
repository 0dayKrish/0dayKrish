    ...
    <>
      ...
      >
        <div className="max-w-[1280px] mx-auto px-2 sm:px-4">
          <nav className="bg-[var(--bg-subtle)] border border-[var(--border-color)] shadow-[2px_2px_0px_var(--shadow-color)] px-3 sm:px-5 h-12 sm:h-13 flex items-center justify-between transition-colors duration-200">
            {/* Logo */}
            <a
              href="#hero"
            ...
            {/* Tablet/Mid-screen Menu Button (when sm is visible but xl is hidden) */}
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-40 bg-black/50 xl:hidden backdrop-blur-xs flex flex-col justify-start pt-18 px-4">
          <div className="bg-[var(--bg-subtle)] border-2 border-[var(--border-color)] shadow-[6px_6px_0px_var(--shadow-color)] p-5 font-mono max-w-sm w-full mx-auto animate-in fade-in zoom-in-95 duration-150">
            <div className="border-b border-[var(--border-color)] pb-2 mb-4 flex items-center justify-between text-[0.65rem] text-[var(--text-secondary)]">
              <span>NAVIGATION.LOG</span>
              <span className="text-[var(--accent-emerald)]">{"// 0daykrish"}</span>
            </div>
            <div className="flex flex-col gap-2">
              {navItems.map((item) => (
                <a
                  ...
                  className="px-3 py-2 border border-[var(--border-color)] bg-[var(--bg-surface)] hover:bg-[var(--text-primary)] hover:text-[var(--bg-primary)] text-[var(--text-primary)] transition-colors text-xs font-bold uppercase no-underline flex items-center justify-between"
                >
