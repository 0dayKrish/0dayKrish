...
export function Navbar() {
  ...
  const navItems = [
    ...
    { label: "HUNT", href: "#vulnerability-hunt" },
      ...
      >
        <div className="max-w-[1280px] mx-auto px-2 sm:px-4">
          <nav className="bg-[var(--bg-subtle)] border border-[var(--border-color)] shadow-[2px_2px_0px_var(--shadow-color)] px-3 sm:px-4 h-12 sm:h-13 flex items-center justify-between transition-colors duration-200">
            ...
            {/* Right Action Cluster: Dossier + ⌘K + ThemeToggle + Contact */}
            <div className="hidden sm:flex items-center gap-2 flex-shrink-0">
              {/* Security Dossier Trigger */}
              <button
                type="button"
                onClick={openSecurityDossier}
                aria-label="Open Security Dossier"
              ...
              >
                <FileText className="w-3 h-3 text-[var(--accent-emerald)]" />
                <span>DOSSIER</span>
              </button>
            ...
            {/* Mobile Actions: Dossier + ⌘K + Theme Toggle + Menu Button */}
            <div className="flex sm:hidden items-center gap-1.5">
              ...
              >
                <FileText className="w-3.5 h-3.5 text-[var(--accent-emerald)]" />
              </button>
              <button
                type="button"
                onClick={openCommandPalette}
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 2xl:hidden backdrop-blur-xs flex flex-col justify-start pt-16 px-4">
          <div className="bg-[var(--bg-subtle)] border-2 border-[var(--border-color)] shadow-[6px_6px_0px_var(--shadow-color)] p-5 font-mono max-w-sm w-full mx-auto max-h-[85vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-150">
            ...
            {/* Security Dossier Trigger inside Drawer */}
            <div className="mb-2">
              <button
                type="button"
                onClick={() => {
                ...
                className="w-full flex items-center justify-between px-3 py-2 border border-[var(--accent-emerald)] bg-[var(--bg-surface)] text-[var(--text-primary)] font-mono text-xs font-bold uppercase cursor-pointer hover:bg-[var(--accent-emerald)] hover:text-white transition-colors"
              >
                <span className="flex items-center gap-2">
                  <FileText className="w-3.5 h-3.5 text-[var(--accent-emerald)]" />
                  <span>SECURITY DOSSIER</span>
                </span>
                <span className="text-[0.6rem] text-[var(--accent-emerald)] font-bold">
                  [01-07]
                </span>
              </button>
            </div>
